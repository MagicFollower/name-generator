...
public class NameGenerator {
    ...
    static final List<String> names = Arrays.asList("fAaliyah", "mAaron", "fAarushi", "fAbagail", "fAbbey", "fAbbi", "fAbbie", "fAbby", "mAbdul", "mAbdullah"
            ...
            , "fAshvini", "fAsia", "fAsma", "mAspen", "fAspen", "mAston", "mTrivic", "fTrivic", "fAstrid", "mAthan", "fAthena", "fAthene"
    ...
    static final List<String> mWords = new ArrayList<>();
    static final List<String> fWords = new ArrayList<>();

    static {
        names.forEach(x -> {
            if (x.startsWith("m")) {
                mWords.add(x.substring(1));
            } else {
                fWords.add(x.substring(1));
            }
        });
    }
    ...
    public static String getName(boolean female) {
        return getName(female, NameFormat.COMMON);
    }

    public static String getName(boolean female, NameFormat nameFormat) {
        String fullName, firstName, secondName;
        if (female) {
            firstName = fWords.get(ThreadLocalRandom.current().nextInt(fWords.size()));
            secondName = fWords.get(ThreadLocalRandom.current().nextInt(fWords.size()));
        } else {
            firstName = mWords.get(ThreadLocalRandom.current().nextInt(mWords.size()));
            secondName = mWords.get(ThreadLocalRandom.current().nextInt(mWords.size()));
        }
